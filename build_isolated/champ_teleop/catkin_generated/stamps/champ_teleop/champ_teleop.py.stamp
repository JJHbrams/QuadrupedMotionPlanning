...
class Teleop:
    def __init__(self):
        ...
        self.usercommand = rospy.Publisher('User_Command', Int16, queue_size = 1)
    ...
    def poll_keys(self):
        ...
        try:
            ...
            while not rospy.is_shutdown():
                # Key input from keyboard(User command)
                # key = self.getKey()
                # Key input automatic
                if(cnt > MAX_CNT):
                    key_id = rd.randrange(0,8)
                    key = keylist[key_id]
                    self.usercommand.publish(key_id)
                    # print(key)
